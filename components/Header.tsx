...
export const Header: React.FC<HeaderProps> = ({ onNavigate }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-zinc-900 py-4 px-6">
      <div className="max-w-6xl mx-auto flex justify-between items-center relative">
        ...
        <button 
          ...
          className="md:hidden flex items-center justify-center w-10 h-10 text-zinc-400 hover:bg-zinc-900 rounded-full transition-colors"
          aria-label="Menu"
        >
        ...
        {isMenuOpen && (
          <div className="absolute top-full right-0 mt-2 w-48 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl py-2 md:hidden animate-in fade-in slide-in-from-top-2 duration-200">
            <button 
              onClick={() => handleMobileNav('contribuir')}
              className="w-full text-left px-5 py-3 text-sm font-semibold text-zinc-300 hover:bg-zinc-800 hover:text-blue-500 transition-colors"
            >
              Contribuir
            </button>
            <button 
            ...
            <div className="border-t border-zinc-800 mt-1 pt-1">
              <button 
                onClick={() => handleMobileNav('contribuir')}
                className="w-full text-left px-5 py-3 text-sm font-black text-blue-500 hover:bg-zinc-800 transition-colors uppercase tracking-widest"
              >
                Apoiar Agora
              </button>
            </div>
